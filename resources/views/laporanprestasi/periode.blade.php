        ...
        <div class="card-header pb-1">
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center gap-2"> <!-- Tambahkan div wrapper ini -->
                    <select id="periode_filter" class="form-select" style="width: 200px; height: 42px; margin-top: 0px;"> <!-- Atur width disini -->
...
$(document).ready(function() {
   ...
   $('#periode_filter').change(function() {
        if ($(this).val()) {
            $('#export_btn').removeClass('btn-secondary').addClass('btn-primary').prop('disabled', false);
        } else {
            $('#export_btn').removeClass('btn-primary').addClass('btn-secondary').prop('disabled', true);
        }
        table.ajax.reload();
    });
    ...
    $('#periode_filter').change(function() {
        ...
        if (periodeId) {
            ...
            $('#export_pdf').off('click').on('click', function() {
                window.location.href = `{{ url('laporan-prestasi/periode/export') }}/${periodeId}?format=pdf`;
            });
            
            $('#export_excel').off('click').on('click', function() {
                window.location.href = `{{ url('laporan-prestasi/periode/export') }}/${periodeId}?format=excel`;
            });
        } else {
